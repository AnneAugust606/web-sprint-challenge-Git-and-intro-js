...
3. Create an object with the following format:
  { 
    name: Gretchyn Hickman, 
    years: 08/14/2002 - 04/14/2022,
    genre: Web Design, 
    nationality: Gay,
    bio: *insert cool bio here*
  }  
4. Return the array
🌟 EXAMPLE: Invoking addArtist(artists, 'John Doe', '1988-2022', 'Full Stack Development', 'African American', 'I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.') should return the artists array with the above object added to the end of the array. */

function addArtist(arr, one, two, three, four, five) {
  const newArtist = {name: one,years: two, genre: three, nationality: four, bio: five,}
  arr.push(newArtist);
  return arr
}
addArtist(artists, "Gretchyn Hickman", "08/14/2002 - 04/14/2022", "Full Stack Web Design", "Gay", "*insert cool bio here*");
...
function artistByCountry(arr, nation) {
  let byCountry = [];
  for (let i = 0; i < arr.length; i++){
    if(arr[i].nationality == nation){
      byCountry.push(arr[i].name);
    }
}
return byCountry;
}
